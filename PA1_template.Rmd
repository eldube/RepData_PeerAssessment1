...
```{r cache=TRUE}
## unzip the source file to extract "activity.csv" datafile and load the data set
unzip(zipfile) 
...
The mean number of steps per day taken by the individual in a 5-minute interval is `r meanTotalNumberofSteps ` is and the median is `r medianTotalNumberofSteps `
...
avgpatternData <- ddply(projData, "intervalFactor", transform, 
...
library(plyr)
filledProjData <-  ddply(projData, .(intervalFactor), transform, 
                                     intervalMean = mean(steps,  na.rm=TRUE))

filledProjData$steps <- with( filledProjData, 
                                  ifelse( is.na(steps), intervalMean , steps) )
...
The revised average number of steps per interval is `r revmeanTotalNumberofSteps ` is and the median is `r revmedianTotalNumberofSteps `. The chosen strategy has not changed the mean and the median has increased.
